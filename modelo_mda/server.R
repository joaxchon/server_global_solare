server <- function(input, output) {
  ...
  get_data <-  reactive({ 
    ...
    con <- dbConnect(MySQL(), user='admin', password='joaxon1234', 
                     ...
                     port=3306)
    rs <- dbSendQuery(con, paste('select fecha, hora, clave_nodo as nodo , pml as precio_mda from pml_mda ',
                                 'where fecha between ',fecha_2,' and ',fecha_1,' and clave_nodo = ', paste('"',as.character(input$node),'"', sep = ""),';'  ,sep=''))
    
    mda_pnd<- fetch(rs, n=-1)
  ...
  df_final <- reactive({
    ...
    prom_sem <- c()
    for(i in 1:24){
      prom_4_sim  <- c(prom_4_sim,mean(dia_4_sim$precio_mda[which(dia_4_sim$hora == i)]))
      prom_2_sim  <- c(prom_2_sim,mean(dia_2_sim$precio_mda[which(dia_2_sim$hora == i)]))
      prom_2_a  <- c(prom_2_a,mean(dia_2_a$precio_mda[which(dia_2_a$hora == i)]))
      prom_sem <- c(prom_sem, mean(sem$precio_mda[which(sem$hora == i)]))
    }
      ...
      if(input$decision == "Dia previo"){
        dia$precio_mda <- dia$precio_mda*(1+input$perc/100)
        precio <- dia$precio_mda
      }
      if(input$decision == "Dia Anterior Similar"){
        dia_1_sim$precio_mda <- dia_1_sim$precio_mda*(1+input$perc/100)
        precio <- dia_1_sim$precio_mda
      }
    ...
    for(j in 1:24){
      ...
      time_series <- data_hour$precio_mda
      ff <- ar(time_series)
      orden <- ff$order
    ...
    df_final <- data.frame("Anterior" = dia$precio_mda,"Anterior similar" = dia_1_sim$precio_mda,"Promedio 4 similares" = prom_4_sim,"Promedio 2 similares" = prom_2_sim,"Promedio 2 anteriores" =prom_2_a,"Promedio semanal" = prom_sem, "Autoregresivo" = prediccion, "hora" = seq(1,24,1))
    #df_final <- data.frame("Anterior" = dia$precio_mda,"Anterior similar" = dia_1_sim$precio_mda,"Promedio 4 similares" = prom_4_sim,"Promedio 2 similares" = prom_2_sim,"Promedio 2 anteriores" =prom_2_a,"Promedio semanal" = prom_sem, "Autoregresivo" = prediccion, "Pronostico" = precio ,"hora" = seq(1,24,1))
