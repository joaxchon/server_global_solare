...
server <- function(input, output) {
  ...
  matriz_pronos <- reactive({
    ...
    con <- dbConnect(MySQL(), user='admin', password='joaxon1234', 
                     ...
                     port=3306)
    rs <- dbSendQuery(con, paste('select * from pronostico_mda where fecha between ',fecha_1, ' and ', fecha_2, ' and nodo = ', nodo_base,' ;',sep = ""))  
    pml_pronos<- fetch(rs, n=-1)
    dbDisconnect(con)
    #print(paste('select * from pronostico_mda where fecha between ',fecha_1, ' and ', fecha_2, ' and nodo = ', nodo_base,' ;',sep = ""))
    #print(pml_pronos)
    pml_pronos
  })
  ...
  output$plot <- renderPlot({
    if(length(matriz_pml()$hora) == length(matriz_pronos()$hora)){
      ggplot(DISPLAY(), aes(x = seq(1,length(DISPLAY()$`hora`)))) + geom_line(aes(y = precio_mda), color = "blue", size = 1)+
        geom_line(aes(y = precio_pronosticado),col = "red", size = 1, alpha  = 0.5)+ 
        geom_ribbon( aes(ymin=precio_mda,ymax=precio_pronosticado), fill="green", alpha=0.1)+
        labs(x = "Horas", y = "MXN")
    }
  })
  ...
  output$table <- renderTable({ 
    if(length(matriz_pml()$hora) == length(matriz_pronos()$hora)){
      DISPLAY() %>% 
        dplyr::select(fecha,hora,precio_mda,precio_pronosticado) %>%
        mutate(precio_mda-precio_pronosticado) %>%
        mutate(porcentaje_error = (precio_mda-precio_pronosticado)/precio_pronosticado * 100)
